...
import { query } from "../../lib/database"; // Import your Neon query function

export async function GET(req) {
  try {
    // Query the database for the most recent post (or all posts if needed)
    const result = await query("SELECT * FROM uploads ORDER BY id DESC LIMIT 1");

    // If there's a new post, send it to the client via SSE
    if (result.rows.length > 0) {
      const newPost = result.rows[0];
      return new Response(`data: ${JSON.stringify(newPost)}\n\n`, {
        headers: {
          "Content-Type": "text/event-stream",
          "Cache-Control": "no-cache",
          "Connection": "keep-alive",
          "Access-Control-Allow-Origin": "*", // Make sure CORS is handled correctly
        },
      });
    }

    // If no new post, you can return an empty response or just do nothing.
    return new Response(null, { status: 204 }); // No new content
  } catch (error) {
    console.error('Error fetching posts:', error);
